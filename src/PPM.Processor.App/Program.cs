...
namespace ImageMain
{
  class Program
  {
    ...
    static void Main(string[] args)
    {
      ...
      //
      // User has selected operation, so create output directory
      //
      try
      {
      ...
      {
        Console.WriteLine("**Error: unable to create output directory: {0}", e.Message);
        return;
      }
      ...
      if (cmd == 1)
      {
        ...
        outfile = "output/" + outfile + "-grayscale.ppm";
      ...
      else if (cmd == 2)
      {
        ...
        string outfile = System.IO.Path.GetFileNameWithoutExtension(filename);
        outfile = "output/" + outfile + "-threshold.ppm";
        Console.WriteLine("Writing '{0}'...", outfile);
        Utility.WriteImageFile(outfile, depth, ref newPixels);
      }
      else if (cmd == 3)
      {
        ...
        string outfile = System.IO.Path.GetFileNameWithoutExtension(filename);
        outfile = "output/" + outfile + "-flip-horz.ppm";
        Console.WriteLine("Writing '{0}'...", outfile);
        Utility.WriteImageFile(outfile, depth, ref newPixels);
      }
      else if (cmd == 4)
      {
        ...
        string outfile = System.IO.Path.GetFileNameWithoutExtension(filename);
        outfile = "output/" + outfile + "-edge.ppm";
        Console.WriteLine("Writing '{0}'...", outfile);
        Utility.WriteImageFile(outfile, depth, ref newPixels);
      }
      else if (cmd == 5)
      {
        ...
        string outfile = System.IO.Path.GetFileNameWithoutExtension(filename);
        outfile = "output/" + outfile + "-rotated.ppm";
        Console.WriteLine("Writing '{0}'...", outfile);
        Utility.WriteImageFile(outfile, depth, ref newPixels);
      }
      else
